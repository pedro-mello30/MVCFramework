...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './cache');				// Directory where images are cached. Left blank it will use the Core temporary directory (which is better for security)
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if (!function_exists ('imagecreatetruecolor')) {
		    return $this->error('GD Library ErrorHelper: imagecreatetruecolor does not exist - please contact your webhost and ask them to install the GD library');
		}
	...
	protected function serveExternalImage(){
		...
		if(! $this->getURL($this->src, $tempfile)){
			...
			$this->debug(3, "ErrorHelper fetching URL: " . $this->lastURLError);
			$this->error("ErrorHelper reading the URL you specified from remote host." . $this->lastURLError);
			return false;
		}
